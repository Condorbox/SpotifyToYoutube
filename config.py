...
def parse_args():
    ...
    parser.add_argument("--playlist-offset", default=int(os.environ.get("PLAYLIST_OFFSET") or 0), type=int, help="Offset for Spotify playlist pagination (default: 0)")
    parser.add_argument("--download",        action="store_true", default=False,        help="Download songs without prompting")
    parser.add_argument("--no-download",     action="store_true",                       help="Skip download without prompting")
...
def _validate(namespace: argparse.Namespace) -> None:
    missing = [hint for attr, hint in _REQUIRED if not getattr(namespace, attr, None)]
    if missing:
        lines = "\n  ".join(missing)
        raise SystemExit(
...
spoti_playlist_id = args.playlist_id
